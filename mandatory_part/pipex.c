...
int	ft_file_to_command_one(char **argv, char **envp, int **pipes)
{
	char	**command1;
	int		infile;
	char	*path;
	...
	(dup2(infile, STDIN_FILENO), dup2(pipes[0][1], STDOUT_FILENO));
...
int	ft_command_one_to_outfile(char **argv, int argc, int **pipes)
{
	...
	if (outfile == -1)
		return (ft_printf("Error when opening the file\n"), 0);
	(dup2(pipes[argc - 5][0], STDIN_FILENO), dup2(outfile, STDOUT_FILENO));
